...
| Code host & gate | **GitHub** (`TBD/living-codex`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
...
| Database | **TBD** (e.g. Supabase, CockroachDB, RDS) — one production project + one development/staging project; migrations applied by agents via CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
...
- **Allow:** `pnpm` scripts, `Node`, `tsc`, `biome`, `TBD` (test runner), `TBD` (DB CLI; dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
