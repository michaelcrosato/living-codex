# AI Operations Plan — living-codex
...
| Code host & gate | **GitHub** (`michaelcrosato/living-codex`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
...
| Database | **None (static files only)** — content is baked static data (JSON packs); no runtime database service; all persistence is browser localStorage for saves | Charter hard scope: no backend services; content is data, not a live database. |
| E2E verification | **Playwright** driven by agents ("test like a human user") against the Vite dev server | Already wired in the repo (`pnpm e2e`); charter states browser smoke CI runs via Playwright. |
...
- **Allow:** `pnpm` scripts, `node`, `tsc`, `biome`, `vitest`, `playwright` (no database CLI — static-file stack), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
