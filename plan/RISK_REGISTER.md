...
| SPEC-29 | **tsgo diverges from tsc** on this codebase, or is treated as the canonical gate prematurely. | Low-Med | tsgo is `--noEmit`-only here (Vite emits) where it's production-ready (>98% compat); **keep `tsc` authoritative** — tsgo is only the accelerator; prove `typecheck:fast` agrees with `typecheck` before relying on it. | Drop the `typecheck:fast` script/devDep; `tsc` is unchanged. |
| SPEC-30 | **Mutation run is slow/flaky** or tempts mass test-writing (scope creep). | Med | Scope to `engine-core` only; incremental + per-test coverage; **report-only** (not in `verify`); record baseline, file survivors to BACKLOG, do NOT fix them all here. | Remove Stryker config + script; zero shipped-code impact. |
| SPEC-31 | **Model-based PBT surfaces a real determinism bug** (good) or a perf-heavy/flaky test (bad). | Low-Med | Bound `numRuns`, pin seed; a true divergence is R1 (shrink+file+fix/revert, never silence); keep the model minimal. | Reduce runs / `.skip` with a filed BACKLOG bug if purely perf. |
| SPEC-32 | **Drama-manager breaks determinism or over-reaches into main plot** — the highest-risk Cycle-3 item. | Med-High | DESIGN NOTE FIRST; pure salience function over `World` (no `Math.random`/`Date.now`); ties via the seeded RNG inside the fold; main-plot storylets excluded by tag; replay-invariant + model-based suite must stay green. | Revert; storylet steering is additive — removal restores the SPEC-11 baseline. |
| SPEC-33 | **New content breaks a quest gate or storylet satisfiability**, or changes a hardcoded count/fingerprint. | Low-Med | `content:validate`/`content:verify` (incl. SPEC-25) gate it; nothing hardcodes the fingerprint/pack-count (verified Cycle 2); golden-master untouched (hand-authored). | Remove the pack dir; content is additive. |
