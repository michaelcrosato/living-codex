...
export function buildCanonIndex(
  ...
  for (const f of registries.factions.values())
    entities.push({ id: f.id, kind: "faction", name: f.name, summary: clip(f.ethos) });
  for (const n of registries.npcs.values())
    entities.push({ id: n.id, kind: "npc", name: n.name, summary: clip(n.bio.role) });
  for (const l of registries.locations.values())
    entities.push({ id: l.id, kind: "location", name: l.name, summary: clip(l.mood) });
  for (const i of registries.items.values())
    entities.push({ id: i.id, kind: "item", name: i.name, summary: clip(i.description) });
  for (const q of registries.quests.values())
    entities.push({ id: q.id, kind: "quest", name: q.title, summary: clip(q.summary) });
